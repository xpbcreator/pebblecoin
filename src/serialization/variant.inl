...
namespace {
  ...
  struct _variant_visitor : public boost::static_visitor<bool>
  {
    Archive &ar;

    _variant_visitor(Archive &a) : ar(a) { }

    template <class T>
    bool operator ()(const T &rv) const
    {
      ar.begin_variant();
      ar.write_variant_tag(variant_serialization_traits<Archive, T>::get_tag());
...
struct serializer<Archive<true>, boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>>
{
  ...
  static bool serialize(Archive<true> &ar, variant_type &v) {
    return boost::apply_visitor(_variant_visitor<Archive<true> >(ar), v);
  }
};
