...
namespace {
  ...
  bool _do_serialize_vector_impl(Archive<false> &ar, std::vector<T> &v)
  {
    size_t cnt;
    ar.begin_array(cnt);
    if (!ar.stream().good())
  ...
  template <template <bool> class Archive, class T>
  bool _do_serialize_vector_impl(Archive<true> &ar, std::vector<T> &v)
  {
    size_t cnt = v.size();
    ar.begin_array(cnt);
    for (size_t i = 0; i < cnt; i++) {
...
{
  return _do_serialize_vector_impl(ar, v);
}
