...
class daemon_cmmands_handler
{
  node_server_t& m_srv;
public:
  daemon_cmmands_handler(node_server_t& srv):m_srv(srv)
  {
...
private:
  epee::srv_console_handlers_binder<node_server_t> m_cmd_binder;
  ...
  bool show_hr(const std::vector<std::string>& args)
  {
	...
	{
	  m_srv.get_payload_object().get_core().get_miner().set_print_hashrate(true);
	}
    return true;
  }
  //--------------------------------------------------------------------------------
  ...
  {
    m_srv.get_payload_object().get_core().get_miner().set_print_hashrate(false);
    return true;
  }
  //--------------------------------------------------------------------------------
  ...
  bool print_block_by_hash(const std::string& arg)
  {
    ...
    {
      cryptonote::block block = blocks.front();
      std::cout << "HEIGHT: " << get_block_height(block) << ENDL;
      print_as_json(block);
    }
    else
    {
