...
void PaymentServer::LoadRootCAs(X509_STORE* _store)
{
    ...
    QString certFile = QString::fromStdString(GetArg(qt_opt::arg_root_certificates));
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
                ...
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid Pebblecoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    emit message(tr("Payment request error"),
        "PaymentServer::processPaymentRequest() NYI",
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    qDebug() << "PaymentServer::fetchPaymentACK NYI";
    return;
    /*
    QNetworkRequest netRequest;
