...
void CoinControlDialog::showMenu(const QPoint &point)
{
    ...
    if(item)
    {
        ...
        if (item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64 characters (this means its a child node, so its not a parent node in tree mode)
        {
            ...
            if (false) //model->isLockedCoin(uint256(item->text(COLUMN_TXHASH).toStdString()), item->text(COLUMN_VOUT_INDEX).toUInt()))
            {
...
QString CoinControlDialog::getPriorityLabel(double dPriority)
{
    ...
    return "CoinControlDialog::getPriorityLabel() NYI";
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    */
    ...
    bool bAllowFree = false; //AllowFree(dPriority);
  
    // actually update labels
    int nDisplayUnit = BitcoinUnits::XPB;
    ...
    toolTip1 += tr("This means a fee of at least %1 per kB is required.").arg(BitcoinUnits::formatWithUnit(nDisplayUnit, CTransaction_nMinTxFee)) + "<br /><br />";
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const auto& coins, mapCoins)
    {
