...
BitcoinGUI::BitcoinGUI(boost::program_options::options_description descOptionsIn, bool fIsTestnet, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        setNumBlocks(clientModel->getDaemonProcessedHeight(), clientModel->getWalletProcessedHeight(),
...
{
    HelpMessageDialog *help = new HelpMessageDialog(descOptions, this);
...
void BitcoinGUI::gotoSupportPage()
{
...
void BitcoinGUI::setNumBlocks(int daemonCount, int walletCount, int nTotalBlocks)
{
    ...
    QDateTime lastDaemonBlockDate = clientModel->getLastDaemonBlockDate();
    QDateTime lastWalletBlockDate = clientModel->getLastWalletBlockDate();
    QDateTime currentDate = QDateTime::currentDateTime();
    int daemonSecs = lastDaemonBlockDate.secsTo(currentDate);
    int walletSecs = lastWalletBlockDate.secsTo(currentDate);

    if(daemonCount < nTotalBlocks)
    {
        tooltip = tr("Validated %1 of %2 (estimated) blocks.").arg(daemonCount).arg(nTotalBlocks);
    }
    else if (walletCount < nTotalBlocks)
    {
        tooltip = tr("Validated %1 blocks and processed %2 of %3 blocks of transaction history.").arg(daemonCount).arg(walletCount).arg(nTotalBlocks);
    }
    else
    {
        tooltip = tr("Validated and processed %1 blocks of transaction history.").arg(nTotalBlocks);
    }

    // Set icon state: spinning if catching up, tick otherwise
    if(daemonSecs < 90*60 && daemonCount >= nTotalBlocks && walletCount >= nTotalBlocks)
    {
    ...
    {
        QString timeBehindText = getBehindText(daemonCount >= nTotalBlocks ? walletSecs : daemonSecs);
        progressBarLabel->setVisible(true);
        if (nTotalBlocks <= 0)
        {
            progressBar->setFormat(tr("Not connected"));
        }
        else if (daemonCount >= nTotalBlocks)
        {
            if (walletCount >= nTotalBlocks)
            {
                progressBar->setFormat(tr("Waiting for newer block"));
            }
            else
            {
                progressBar->setFormat(tr("Blocks synced, wallet is %1 behind").arg(timeBehindText));
            }
        }
        else
        {
        ...
        if(daemonCount + walletCount != prevBlocks)
        {
        ...
        prevBlocks = daemonCount + walletCount;
        ...
        if (daemonCount >= nTotalBlocks)
        {
            tooltip += tr("Last processed block was generated %1 ago.").arg(timeBehindText);
        }
        else
        {
