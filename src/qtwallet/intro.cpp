...
{
    return GUIUtil::boostPathToQString(tools::get_default_data_dir());
}
...
void Intro::pickDataDirectory()
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || GetArg(qt_opt::arg_choose_data_dir))
    {
     ...
     */
    if(dataDir != getDefaultDataDirectory())
        SoftSetArg(command_line::arg_data_dir, GUIUtil::qstringToBoostPath(dataDir).string()); // use OS locale for path setting
}
