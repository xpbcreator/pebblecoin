...
struct TxLessThan
{
    ...
    bool operator()(const TransactionRecord &a, const std::string &b) const
    {
        return a.hash < b;
    }
    bool operator()(const std::string &a, const TransactionRecord &b) const
    {
        return a < b.hash;
    }
};
...
public:
    ...
    TransactionRecord *index(int idx)
    {
        if(idx >= 0 && idx < cachedWallet.size())
        {
            ...
            LOG_PRINT_L4("TRY_LOCK(cs_main) TransactionTablePriv::index");
            TRY_LOCK(cs_main, lockMain);
            if(lockMain)
            {
                LOG_PRINT_L4("TRY_LOCK(cs_main) TransactionTablePriv::index acquired");
                LOG_PRINT_L4("TRY_LOCK(cs_wallet) TransactionTablePriv::index");
                TRY_LOCK(wallet->cs_wallet, lockWallet);
                LOG_PRINT_L4("TRY_LOCK(cs_wallet) TransactionTablePriv::index acquired");
                if(lockWallet && rec->statusUpdateNeeded())
                {
                    auto mi = wallet->mapWallet.find(GetCryptoHash(rec->hash));
    ...
    {
        {
            LOG_PRINT_L4("LOCK2(cs_main, wallet->cs_wallet) TransactionTablePriv::describe");
            LOCK2(cs_main, wallet->cs_wallet);
            LOG_PRINT_L4("LOCK2(cs_main, wallet->cs_wallet) TransactionTablePriv::describe acquired");
            auto mi = wallet->mapWallet.find(GetCryptoHash(rec->hash));
            if(mi != wallet->mapWallet.end())
            {
...
{
    std::string updated;
    updated = hash.toStdString(); //.SetHex(hash.toStdString());
