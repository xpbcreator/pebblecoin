...
#include "interface/main.h" // for CTransaction::nMinTxFee and MAX_SCRIPTCHECK_THREADS
    ...
    fProxyIpValid(true)
{
    ...
    // Main elements init
    ui->databaseCache->setMinimum(1337); //nMinDbCache);
    ui->databaseCache->setMaximum(6666); //nMaxDbCache);
    ui->threadsScriptVerif->setMinimum(-(int)boost::thread::hardware_concurrency());
    ui->threadsScriptVerif->setMaximum(1337); //MAX_SCRIPTCHECK_THREADS);
    ...
    ui->transactionFee->setSingleStep(CTransaction_nMinTxFee);
...
void OptionsDialog::doProxyIpChecks(QValidatedLineEdit *pUiProxyIp, int nProxyPort)
{
    ...
    if (!strAddrProxy.empty()) //(fProxyIpValid = LookupNumeric(strAddrProxy.c_str(), addrProxy)))
    {
        disableOkButton();
