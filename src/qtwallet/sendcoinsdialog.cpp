        ...
        if(entry)
        {
            if(entry->validate())
            {
                ...
                if (model->getAddressTableModel())
                    model->getAddressTableModel()->addRow(AddressTableModel::Send, entry->getValue().label, entry->getValue().address, true);
            }
            else
            {
                valid = false;
...
{
    ui->labelCoinControlChangeLabel->setText("SendCoinsDialog::coinControlChangeEdited NYI");
    return;
    /*
...
void SendCoinsDialog::coinControlUpdateLabels()
{
    ...
    if (false) //CoinControlDialog::coinControl->HasSelected())
    {
