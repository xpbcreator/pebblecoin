...
namespace command_line
{
  ...
  template<typename T, bool required>
  bool has_defaulted_arg(const boost::program_options::variables_map& vm, const arg_descriptor<T, required>& arg)
  {
    if (vm.find(arg.name) == vm.end())
      return false;
  ...
  {
    if (vm.find(arg.name) == vm.end())
      throw std::runtime_error(std::string("Argument not present: ") + arg.name);
