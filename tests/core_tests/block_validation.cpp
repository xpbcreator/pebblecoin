...
namespace
{
                          ...
                          size_t new_block_count, const block blk_last, const account_base& miner_account)
  {
    ...
    {
      block blk_next;
      difficulty_type diffic = next_difficulty(0, timestamps, cummulative_difficulties);
...
bool gen_block_ts_not_checked::generate(std::vector<test_event_entry>& events) const
{
  ...
  // checking can create block messes up the difficulty, this test doesn't check that anyway
...
bool gen_block_invalid_nonce::generate(std::vector<test_event_entry>& events) const
{
  ...
  difficulty_type diffic = next_difficulty(0, timestamps, commulative_difficulties);
  assert(1 < diffic);
...
bool gen_block_unlock_time_is_timestamp_in_future::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_MINER_TX_MANUALLY(miner_tx, blk_0);
  miner_tx.unlock_time = blk_0.timestamp + 3 * CRYPTONOTE_MINED_MONEY_UNLOCK_WINDOW * cryptonote::config::difficulty_blocks_estimate_timespan();
...
bool gen_block_height_is_low::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_MINER_TX_MANUALLY(miner_tx, blk_0);
  boost::get<txin_gen>(tx_tester(miner_tx).vin[0]).height--;
...
bool gen_block_miner_tx_with_txin_to_key::generate(std::vector<test_event_entry>& events) const
{
  ...
  tx_tester(miner_tx).vin[0] = tmp_tx.ins()[0];
...
bool gen_block_miner_tx_out_is_small::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_MINER_TX_MANUALLY(miner_tx, blk_0);
  tx_tester(miner_tx).vout[0].amount /= 2;
...
bool gen_block_invalid_binary_format::generate(std::vector<test_event_entry>& events) const
{
  ...
  do
  {
    ...
    diffic = next_difficulty(0, timestamps, cummulative_difficulties);
  ...
  blk_last = boost::get<block>(events.back());
  MAKE_TX(events, tx_0, miner_account, miner_account, 120, boost::get<block>(events[1]));
...
bool gen_block_invalid_binary_format::corrupt_blocks_boundary(core_t& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  m_corrupt_blocks_begin_idx = ev_index + 1;
  return true;
}

bool gen_block_invalid_binary_format::check_all_blocks_purged(core_t& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
