...
bool gen_block_reward::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_NEXT_BLOCK(events, blk_2pre, blk_1, miner_account);
  MAKE_NEXT_BLOCK(events, blk_2, blk_2pre, miner_account);
  DO_CALLBACK(events, "mark_checked_block");
  ...
  transaction tx_0(construct_tx_with_fee(events, blk_5, miner_account, bob_account, 1, 3 * TESTS_DEFAULT_FEE));
  ...
  txs_0.push_back(construct_tx_with_fee(events, blk_5, miner_account, bob_account, 1, 5 * TESTS_DEFAULT_FEE));
  txs_0.push_back(construct_tx_with_fee(events, blk_5, miner_account, bob_account, 1, 7 * TESTS_DEFAULT_FEE));
  MAKE_NEXT_BLOCK_TX_LIST(events, blk_7, blk_6, miner_account, txs_0);
  DO_CALLBACK(events, "mark_checked_block");

  // Test: block reward == transactions fee
  {
    transaction tx_1 = construct_tx_with_fee(events, blk_5, miner_account, bob_account, 1, 11 * TESTS_DEFAULT_FEE);
    transaction tx_2 = construct_tx_with_fee(events, blk_5, miner_account, bob_account, 1, 13 * TESTS_DEFAULT_FEE);
...
bool gen_block_reward::mark_invalid_block(core_t& /*c*/, size_t ev_index, const std::vector<test_event_entry>& /*events*/)
{
  m_invalid_block_index = ev_index + 1;
  return true;
}

bool gen_block_reward::mark_checked_block(core_t& /*c*/, size_t ev_index, const std::vector<test_event_entry>& /*events*/)
{
...
bool gen_block_reward::check_block_rewards(core_t& /*c*/, size_t /*ev_index*/, const std::vector<test_event_entry>& events)
{
  ...
  for (size_t i = 0; i < 5; ++i)
  {
    ...
    CHECK_EQ(cryptonote::get_base_reward(get_block_height(blk_i)), get_tx_out_amount(blk_i.miner_tx));
  }

  block blk_n1 = boost::get<block>(events[m_checked_blocks_indices[5]]);
  CHECK_EQ(cryptonote::get_base_reward(get_block_height(blk_n1)) + 3 * TESTS_DEFAULT_FEE,
           get_tx_out_amount(blk_n1.miner_tx));

  block blk_n2 = boost::get<block>(events[m_checked_blocks_indices[6]]);
  CHECK_EQ(cryptonote::get_base_reward(get_block_height(blk_n2)) + (5 + 7) * TESTS_DEFAULT_FEE,
           get_tx_out_amount(blk_n2.miner_tx));
