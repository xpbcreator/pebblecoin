...
bool gen_tx_big_version::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_GENESIS_BLOCK(events, blk_n2, miner_account, ts_start);
  MAKE_NEXT_BLOCK(events, blk_n1, blk_n2, miner_account);
  MAKE_NEXT_BLOCK(events, blk_0, blk_n1, miner_account);
...
bool gen_tx_signatures_are_invalid::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_TX_MIX(events, tx_1, bob_account, miner_account, 1, 1, blk_2);
  events.pop_back();
