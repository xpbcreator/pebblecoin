                     ...
                     const block& blk_head, const cryptonote::account_base& from, uint64_t amount, size_t nmix)
{
    ...
    LOG_PRINT_YELLOW("fill_tx_sources: Found " << outs_mine.size() << " different amounts in outs_mine",
                     LOG_LEVEL_0);
    
    // Iterate in reverse is more efficiency
    uint64_t num_outs_mine = 0;
    uint64_t sources_amount = 0;
    bool sources_found = false;
    BOOST_REVERSE_FOREACH(const map_output_t::value_type o, outs_mine)
    {
        num_outs_mine += o.second.size();
        for (size_t i = 0; i < o.second.size() && !sources_found; ++i)
        {
            ...
            if (oi.spent)
            {
                LOG_PRINT_YELLOW("Skipping spent output_index", LOG_LEVEL_0);
                continue;
            }
            ...
            {
                LOG_PRINT_YELLOW("Skipping couldn't fill_output_entries", LOG_LEVEL_0);
                continue;
            }
    ...
    if (!sources_found)
        LOG_ERROR("fill_tx_sources: sources were not found, found only " << sources_amount
                  << "/" << amount << " num_outs_mine=" << num_outs_mine);
