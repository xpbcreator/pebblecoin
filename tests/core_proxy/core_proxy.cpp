...
int main(int argc, char* argv[])
{
  ...
  LOG_PRINT_L0("Denitializing shared boulderhash state...");
...
bool tests::proxy_core::handle_incoming_block(const cryptonote::blobdata& block_blob, cryptonote::block_verification_context& bvc, bool update_miner_blocktemplate) {
    ...
    if (!get_block_longhash(b, lh, 0, crypto::g_boulderhash_state, false))
    {
        cerr << "Failed to get block longhash" << endl;
        return false;
    }
    cout << lh << endl;
...
bool tests::proxy_core::init(const boost::program_options::variables_map& /*vm*/) {
    if (!generate_genesis_block(m_genesis))
      return false;
    crypto::hash h = get_block_hash(m_genesis);
    crypto::hash lh;
    if (!get_block_longhash(m_genesis, lh, 0, crypto::g_boulderhash_state, false))
        return false;
...
crypto::hash tests::proxy_core::get_block_id_by_height(uint64_t height)
{
    BOOST_FOREACH(const auto& item, m_hash2blkidx)
    {
